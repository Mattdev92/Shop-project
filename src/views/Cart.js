      ...
      <>
        <MainTemplate>
          <Wrapper sidebaropen={sidebarOpen}>
            {productQuantity !== 0 && (
              <CourtTitle>Artykuły w koszyku</CourtTitle>
            )}
            {Object.keys(productTab).map((key, number) => {
              return (
                <ItemWrapper
                  key={key}
                  ref={(div) => (this.myItem[number] = div)}
                >
                  <Button
                    onClick={() =>
                      DeleteClicked(key, productTab[key].productInfo)
                    }
                    type="delete"
                  >
                    -
                  </Button>
                  <Img src={productTab[key].productImage} alt="error" />
                  ...
                  <PriceSection>{productTab[key].QuantityInCart}</PriceSection>
                  <PriceSection>{productTab[key].TotalPrice} zł</PriceSection>
                </ItemWrapper>
              );
            })}
            <AllProductPrice>
            ...
            {productQuantity !== 0 && (
              <NavLink to="/Buy">
                <Button type="ZAMAWIAM">ZAMAWIAM</Button>
              </NavLink>
            )}
          </Wrapper>
        </MainTemplate>
